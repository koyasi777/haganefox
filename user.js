...
user_pref("browser.urlbar.mdn.featureGate", false);            // MDN [FF117+] [HIDDEN PREF]
user_pref("browser.urlbar.pocket.featureGate", false);         // Pocket [FF116+] [DEFAULT: false]
user_pref("browser.urlbar.weather.featureGate", false);        // weather [FF108+] [DEFAULT: false]
 ...
 * [CUSTOM] Set to false to retain history and restore previous session on startup ***/
 ...
 * [CUSTOM] Setting this to `false` will preserve login sessions. ***/
...
* [CUSTOM] Explicitly set to false to prevent future weakening of FPP for specific sites based on remote server instructions ***/
 ...
 * [CUSTOM] Although RFP is powerful, it significantly impacts usability—causing display issues on websites, fixing the timezone to GMT, etc.—so it is disabled here. ***/
 ...
 * [PURPOSE] Prevent automatic restoration of previous tabs after Firefox crash detection
 * [REASON] Avoid unintended exposure of browsing content, especially on shared or public machines
 * [CUSTOM] Enabling this (true) is practical for normal privacy levels. The default is true, and we explicitly set it here.
 *          Set to false if prioritizing security or anonymity
 */
 ...
 * - [CONFIG STRATEGY] Arkenfox officially supports ETP strict mode (2701), which sets these at runtime
 * - [POLICY] However, we explicitly define them for clarity, auditability, and resilience against future changes
 * - [NOTE] fingerprintingProtection (FPP) is ignored when RFP (4501) is enabled */
// ETP strict: cookie partitioning
user_pref("network.cookie.cookieBehavior", 5); // [DEFAULT: 5 (ETP strict)]
...
// Fingerprinting Protection (FPP) - used since RFP is disabled
// - FPP has fewer compatibility issues than RFP and limits key APIs like Canvas, Audio, WebGL
// - Recommended as a supplement since ETP strict does not block all fingerprinting vectors
user_pref("privacy.fingerprintingProtection", true); // [FF114+] [ETP FF119+]
// OCSP cache partitioning (explicit)
user_pref("privacy.partition.network_state.ocsp_cache", true); // [DEFAULT: true]
// Query stripping
user_pref("privacy.query_stripping.enabled", true); // [FF101+]
// Tracking protection modules (enabled by ETP strict but declared for clarity)
 ...
 * This section contains settings that are not part of the original arkenfox user.js,
 * but have been intentionally added for practicality, quality of life, and enhanced security.
 * Explicitly separated from other sections to maintain consistency across updates.
 */


/* [Imported from Betterfox] --- Start --- */

// Enable userChrome.css and userContent.css (essential for advanced UI customization)
user_pref("toolkit.legacyUserProfileCustomizations.stylesheets", true);

// Completely disable the Pocket feature
user_pref("extensions.pocket.enabled", false);

// Disable fullscreen transition animations and warnings to speed up display
 ...
 *   Designed for developers and advanced users seeking fine-grained UX control */
user_pref("browser.preferences.moreFromMozilla", false);         // Hide "More from Mozilla" in preferences
user_pref("browser.download.open_pdf_attachments_inline", true); // Display PDFs within Firefox instead of external viewer
user_pref("browser.bookmarks.openInTabClosesMenu", false);       // Keep menu open after opening bookmark
user_pref("editor.truncate_user_pastes", false);                 // Preserve formatting when pasting text
user_pref("browser.menu.showViewImageInfo", true);               // Restore "View Image Info" in context menu
user_pref("findbar.highlightAll", true);                         // Highlight all search matches
user_pref("layout.word_select.eat_space_to_next_word", false);   // Do not include trailing space on double-click selection

/* [Privacy/Security] Passwords and telemetry
 * [PURPOSE] Prevent saving of credentials in private browsing and disable telemetry data uploads to Mozilla,
 *   enhancing privacy levels. */
// Disable password saving in private mode.
// Note: This is redundant if signon.rememberSignons=false (see 5003), but included for clarity.
 ...
 * - Compatible with MLS clients (e.g., Firefox, Geoclue)
 * [SOURCE] https://beacondb.net / HN: https://news.ycombinator.com/item?id=40895672
 *   These settings may conflict with SECTION 0200 and pref 7001.
 *   To enable network geolocation, enable this setting and also set:
...
/* [Other Original Custom Settings]  */

/* [Security] Disable autofill on HTTP pages
 * - Prevent autofilling passwords on insecure HTTP pages, even if autofillForms is enabled in the future
 * - Effective against MITM and phishing threats */
user_pref("signon.autofillForms.http", false);

/* [Security] Enable Fission: Site isolation
 * [PURPOSE] Improve memory isolation, crash resistance, and security by isolating processes per origin.
 *   A modern defense against cross-site attacks and memory leaks between processes.
 * [NOTE] Default is true since Firefox 94+. Explicitly set to clarify intent. */
user_pref("fission.autostart", true);

/* [Privacy] Disable telemetry submission during DoH confirmation
 * - Firefox sends confirmation requests before establishing DoH connections, with results reported to Mozilla.
 * - In privacy-focused setups, this telemetry can be safely blocked without affecting DoH functionality.
 * - Disabling has no effect on DNS-over-HTTPS itself. */
